...
import { matchesKey } from '../atoms/keys.js';

/**
 * Tree node data
 */
export interface TreeViewTreeNode {
  /** Unique identifier */
  id: string;
  /** Display label */
  label: string;
  /** Child nodes */
  children?: TreeViewTreeNode[];
...
export class TreeView implements UIElement, InteractiveElement {
  ...
  private data: TreeViewTreeNode[];
  private onSelect?: (node: TreeViewTreeNode) => void;
  private onToggle?: (node: TreeViewTreeNode, expanded: boolean) => void;
  ...
  private visibleNodes: Array<{ node: TreeViewTreeNode; depth: number }> = [];
  ...
  constructor(options: TreeViewOptions) {
    ...
    const ensureExpanded = (nodes: TreeViewTreeNode[]) => {
  ...
  private recomputeVisible(): void {
    this.visibleNodes = [];
    const traverse = (nodes: TreeViewTreeNode[], depth: number) => {
  ...
  serializeState(): any {
    ...
    const walk = (nodes: TreeViewTreeNode[]) => {
      for (const n of nodes) {
