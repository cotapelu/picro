...
export class MemoryScorer {
    ...
    _compiledQuery = '';
    _compiledQueryWords = [];
    _compiledSynonymRegexes = new Map();
    getSynonyms(word) {
    ...
    score(memory, query, currentProject) {
        ...
        if (hoursAgo < 1)
            recencyFactor = 3.0; // Last hour: triple (experiment)
        else if (hoursAgo < 6)
            recencyFactor = 1.5; // Last 6 hours: 1.5x
        else if (hoursAgo < 24)
            recencyFactor = 1.2; // Last 24 hours: 1.2x
        else if (hoursAgo < 72)
            recencyFactor = 1.1; // Last 3 days: 1.1x
        else if (hoursAgo < 168)
            recencyFactor = 1.05; // Last week: 1.05x
...
function computeBM25(query, memories, k1 = 1.2, b = 0.75) {
    ...
    const tfs = Array.from({ length: N }, () => new Array(queryTerms.length).fill(0));
    for (let i = 0; i < N; i++) {
...
export class MemoryRetriever {
     ...
     */
    search(memories, query, currentProject, limit) {
        if (!memories.length || !query)
            return [];
     ...
     */
    fuzzySearch(memories, query, currentProject, limit = 3) {
        if (!memories.length || !query)
            return [];
