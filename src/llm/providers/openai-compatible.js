...
function buildParams(model, context, options, compat) {
    ...
    messages = messages.map(msg => {
        if (msg.role !== 'assistant')
            return msg;
        const asst = msg;
        const newContent = asst.content.map((block) => {
    ...
    for (const msg of messages) {
        ...
        if (msg.role === 'user') {
            ...
            const toolCalls = asst.content.filter((b) => b.type === 'toolCall');
        ...
        for (let i = params.messages.length - 1; i >= 0; i--) {
            ...
            if (msg.role === 'user' || msg.role === 'assistant') {
                if (typeof msg.content === 'string') {
                    ...
                    if (textPart)
                        textPart.cache_control = { type: 'ephemeral' };
                }
                break;
            }
        }
    ...
    if (adjustedCtx.tools?.length) {
        params.tools = adjustedCtx.tools.map((tool) => ({
            type: 'function',
            function: {
                name: tool.name,
    ...
    else if (messages.some((msg) => msg.role === 'toolResult' ||
        (msg.role === 'assistant' && msg.content.some((b) => b.type === 'toolCall')))) {
        params.tools = [];
    }
...
function mapFinishReason(reason) {
    ...
    switch (reason) {
        case 'stop':
        case 'end': return { reason: 'stop' };
        case 'length': return { reason: 'length' };
        case 'tool_calls':
        case 'function_call': return { reason: 'toolUse' };
...
function calcUsage(rawUsage, model) {
        ...
        try {
            ...
            for await (const chunk of responseStream) {
                ...
                if (choice.finish_reason) {
                    ...
                    if (res.message)
                        output.errorMessage = res.message;
                }
                ...
                // Reasoning delta
                const reasoningKey = ['reasoning_content', 'reasoning', 'reasoning_text'].find(k => delta[k]);
                if (reasoningKey) {
                    ...
                    currentBlock.thinking += delta[reasoningKey];
                    eventStream.push({ type: 'thinking_delta', contentIndex: blocks.length - 1, delta: delta[reasoningKey], partial: output });
                }
                // Tool call delta
                if (delta.tool_calls) {
            ...
            if (chunkUsage)
                output.usage = calcUsage(chunkUsage, model);
