...
describe('Agent config and internal methods', () => {
  describe('createLogger', () => {
    it('creates silent logger when enableLogging is false', () => {
      const agent = new Agent(undefined as any, [], defaultConfig({ enableLogging: false }));
    ...
    it('creates verbose logger when enableLogging and verbose are true', () => {
      ...
      const agent = new Agent(undefined as any, [], defaultConfig({ enableLogging: true, verbose: true }));
      // Trigger an event to verify logger is attached - event type must match switch case
      (agent as any).emitter.emit({ type: 'agent:start', timestamp: Date.now(), round: 1 });
    ...
    it('creates verbose logger with different event types', () => {
      ...
      (agent as any).emitter.emit({ type: 'agent:end', timestamp: Date.now(), round: 1 });
      (agent as any).emitter.emit({ type: 'turn:end', timestamp: Date.now(), round: 1, toolCallsExecuted: 2 });
      (agent as any).emitter.emit({ type: 'tool:error', timestamp: Date.now(), round: 1, toolName: 'test' });
      (agent as any).emitter.emit({ type: 'error', timestamp: Date.now(), round: 1, message: 'test error' });
  ...
  describe('_resolveConfig (used in constructor)', () => {
    it('maps toolExecutionStrategy to toolExecutionMode when toolExecutionMode not provided', () => {
      ...
      const config = (agent as any)._resolveConfig({ toolExecutionStrategy: 'sequential' });
      expect(config.toolExecutionMode).toBe('sequential');
    });

    it('maps toolExecutionStrategy parallel to toolExecutionMode', () => {
      const agent = new Agent(undefined as any, [], minimalConfig());
      const config = (agent as any)._resolveConfig({ toolExecutionStrategy: 'parallel' });
    ...
    it('merges provided overrides with defaults', () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      const config = (agent as any).resolveConfig({ maxRounds: 20, verbose: true });
  ...
  describe('_convertToolsToLlm', () => {
    it('converts tools to LLM format', () => {
      ...
      const tools: AgentTool[] = [
        ...
        { name: 'tool3', description: 'Tool 3', parameters: { type: 'object', properties: { a: { type: 'string' } } } },
      ];
      const result = (agent as any)._convertToolsToLlm(tools);
      expect(result).toHaveLength(3);
      expect(result[0]).toEqual({ name: 'tool1', description: 'Tool 1', parameters: { type: 'object', properties: {} } });
      expect(result[1]).toEqual({ name: 'tool2', description: '', parameters: {} });
      expect(result[2].parameters).toEqual({ type: 'object', properties: { a: { type: 'string' } } });
    });
  ...
  describe('_llmComplete and _llmStream with getApiKey', () => {
    it('_llmComplete calls getApiKey when provided', async () => {
      ...
      const agent = new Agent(undefined as any, [], defaultConfig({ getApiKey }));
      ...
      const mockComplete = vi.spyOn(await import('../llm/index.js'), 'complete').mockResolvedValue({
        content: 'response',
        stopReason: 'stop',
    ...
    it('_llmStream calls getApiKey when provided', async () => {
      ...
      const mockStream = vi.spyOn(await import('../llm/index.js'), 'stream').mockResolvedValue({});
  ...
  describe('constructor edge cases', () => {
    ...
    it('prefers toolExecutionMode over toolExecutionStrategy', () => {
      const config = defaultConfig({ toolExecutionMode: 'sequential', toolExecutionStrategy: 'parallel' });
      const agent = new Agent(undefined as any, [], config);
      expect((agent as any).config.toolExecutionMode).toBe('sequential');
    });

    it('maps steeringMode all to queueMode all when queueMode not provided', () => {
      const config = defaultConfig({ steeringMode: 'all', queueMode: undefined });
      const agent = new Agent(undefined as any, [], config);
    ...
    it('maps steeringMode one-at-a-time to queueMode one-at-a-time when queueMode not provided', () => {
      const config = defaultConfig({ steeringMode: 'one-at-a-time', queueMode: undefined });
      const agent = new Agent(undefined as any, [], config);
    ...
    it('defaults loopStrategy to simple when toolExecutionMode is sequential', () => {
      const config = defaultConfig({ toolExecutionMode: 'sequential', loopStrategy: undefined });
      const agent = new Agent(undefined as any, [], config);
      expect((agent as any).strategy.constructor.name).toBe('SimpleLoopStrategy');
    });

    it('defaults loopStrategy to react when toolExecutionMode is parallel', () => {
      const config = defaultConfig({ toolExecutionMode: 'parallel', loopStrategy: undefined });
      const agent = new Agent(undefined as any, [], config);
      expect((agent as any).strategy.constructor.name).toBe('ReActLoopStrategy');
    });
  });
  ...
  describe('setModel with model', () => {
    ...
    it('clears providers when setModel(undefined)', () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      agent.setLLMProvider(async () => ({ content: '', stopReason: 'stop', usage: { input: 0, output: 0, totalTokens: 0, cost: { input: 0, output: 0, total: 0 } }, toolCalls: [] }));
  ...
  describe('execute and streamExecute', () => {
    it('execute calls runner.run with correct arguments', async () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      agent.setLLMProvider(async () => ({ content: 'done', stopReason: 'stop', usage: { input: 1, output: 1, totalTokens: 2, cost: { input: 0, output: 0, total: 0 } }, toolCalls: [] }));
      ...
      const turns = [{ role: 'user' as const, content: [{ type: 'text' as const, text: 'test' }], timestamp: Date.now() }];
      await (agent as any).execute(turns, undefined);
      
      expect(spy).toHaveBeenCalledWith('', agent.steeringQueue, agent.followUpQueue, undefined, turns);
    });
    ...
    it('streamExecute calls runner.stream with correct arguments', async () => {
      ...
      expect(agent.runner.stream).toHaveBeenCalledWith('', agent.steeringQueue, agent.followUpQueue, undefined, turns);
    });
  });
});