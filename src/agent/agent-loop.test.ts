...
describe("AgentLoop", () => {
  ...
  describe("stream", () => {
    it("should stream events and return final result", async () => {
      ...
      const mockStreamProvider = async function* (
        prompt: string,
        tools: any[],
        options?: any,
      ) {
  ...
  describe("run with tool calls", () => {
    it("executes tools and includes results in state", async () => {
      ...
      const resultContent = (successTool!.content as any[]).find(
        (c: any) => c.type === "text",
      );
    ...
    it("handles tool execution errors gracefully", async () => {
      ...
      const errContent = (errTurn!.content as any[]).find(
        (c: any) => c.type === "text",
      );
  ...
  describe("run with steering queue", () => {
    it("drains steering queue into history", async () => {
      ...
      const state = loop.getState();
      const steeringTurns = state.history.filter(
        (t) =>
          t.role === "user" && (t as any).content?.[0]?.text === "steering",
      );
  ...
  describe("run with initialTurns", () => {
    it("includes initialTurns in history", async () => {
      const initialTurns = [
        ...
        {
          role: "assistant",
          content: [{ type: "text", text: "prev resp" }],
          timestamp: Date.now(),
        } as any,
      ];
      loop = new AgentLoop(
  ...
  describe("run: transformPrompt", () => {
    it("applies strategy.transformPrompt", async () => {
      const strategyWithTransform: LoopStrategy = {
        shouldContinue: () => false,
        formatResults: () => "",
        transformPrompt: (p) => p + " [transformed]",
      } as any;
      loop = new AgentLoop(
        config,
        emitter,
  ...
  describe("run: max rounds", () => {
    it("stops after maxRounds and returns max_rounds error", async () => {
      const infiniteStrategy: LoopStrategy = {
        shouldContinue: () => true,
        formatResults: () => "",
      } as any;
  ...
  describe("strategy.shouldContinue", () => {
    it("breaks after tools when shouldContinue false", async () => {
      ...
      const strategyFalse: LoopStrategy = {
        shouldContinue: () => false,
        formatResults: () => "",
      } as any;
      const customLLMComplete = async (
        ...
        toolCalls: [{ id: "c1", name: "e", arguments: {} }],
        raw: {},
      });
      loop = new AgentLoop(
        config,
    ...
    it("continues when shouldContinue true until max rounds", async () => {
      ...
      const strategyTrue: LoopStrategy = {
        shouldContinue: () => true,
        formatResults: () => "",
      } as any;
      const customLLMComplete = async (
        ...
        toolCalls: [{ id: "c2", name: "e", arguments: {} }],
        raw: {},
      });
      loop = new AgentLoop(
        { ...config, maxRounds: 2 },
  ...
  describe("tool call with toolCallId preservation", () => {
    it("preserves toolCallId in tool turn", async () => {
      ...
      const customLLMComplete = async (
        ...
        toolCalls: [{ id: "callABC", name: "g", arguments: {} }],
        raw: {},
      });
      loop = new AgentLoop(
        config,
  ...
  describe("prepareNextTurn hook", () => {
    ...
    it("receives correct context after tool call", async () => {
      ...
      const customLLM = async (
        ...
        if (callCount === 1) {
          return {
            ...
            toolCalls: [
              { id: "call1", name: "echo", arguments: { msg: "hello" } },
            ],
          };
        } else {
          return {
      ...
      expect(call.toolResults[0].content[0].text).toContain(
        'echo: {\"msg\":\"hello\"}',
      );
  ...
  describe("getSteeringMessages hook", () => {
    it("injects steering turns from hook", async () => {
      const hook = vi
        ...
        .mockResolvedValue([
          {
            role: "user",
            content: [{ type: "text", text: "steering from hook" }],
            timestamp: Date.now(),
          } as any,
        ]);
      ...
      const steeringTurns = state.history.filter(
        (t) =>
          t.role === "user" &&
          (t as any).content[0]?.text === "steering from hook",
      );
    ...
    it("falls back to steeringQueue when hook not provided", async () => {
      ...
      steeringQueue.enqueue({
        role: "user",
        content: [{ type: "text", text: "queue steering" }],
        timestamp: Date.now(),
      } as any);
      const followUpQueue = new MessageQueue();
      const customLLM = async (
      ...
      expect(result.success).toBe(true);
      const state = loop.getState();
      const queueTurns = state.history.filter(
        (t) => (t as any).content[0]?.text === "queue steering",
      );
    ...
    it("ignores steeringQueue when hook provided", async () => {
      const hook = vi
        ...
        .mockResolvedValue([
          {
            role: "user",
            content: [{ type: "text", text: "hook turn" }],
            timestamp: Date.now(),
          } as any,
        ]);
      const config = createTestConfig({ getSteeringMessages: hook });
      ...
      steeringQueue.enqueue({
        role: "user",
        content: [{ type: "text", text: "queue turn" }],
        timestamp: Date.now(),
      } as any);
      const followUpQueue = new MessageQueue();
      const customLLM = async (
      ...
      expect(result.success).toBe(true);
      const state = loop.getState();
      const hookTurn = state.history.find(
        (t) => (t as any).content[0]?.text === "hook turn",
      );
      const queueTurn = state.history.find(
        (t) => (t as any).content[0]?.text === "queue turn",
      );
    ...
    it("does not crash when hook throws error", async () => {
      ...
      expect(hook).toHaveBeenCalledTimes(1);
      expect(consoleSpy).toHaveBeenCalledWith(
        "getSteeringMessages hook error:",
        expect.any(Error),
      );
      consoleSpy.mockRestore();
    });
  ...
  describe("multi-modal tool results", () => {
    it("handles tool returning TextBlock[] and ImageBlock[]", async () => {
      const toolCalls: any[] = [];
      ...
      const config = createTestConfig({ maxRounds: 2 });
      const steeringQueue = new MessageQueue();
      const followUpQueue = new MessageQueue();
      let callCount = 0;
      const customLLM = async (
        ...
        if (callCount === 1) {
          return {
            ...
            toolCalls: [{ id: "t1", name: "image_tool", arguments: {} }],
          };
        } else {
          return {
            content: "final",
      ...
      expect(result.success).toBe(true);
      const state = loop.getState();
      const toolTurn = state.history.find(
        (t) => t.role === "tool" && t.toolName === "image_tool",
      );
      expect(toolTurn).toBeDefined();
      const content = toolTurn!.content as any[];
      ...
      // getResultText should include text parts only
      const text = loop["getResultText"](toolTurn as any); // private access hack
