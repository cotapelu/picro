...
function mockLLM(response: string): (prompt: string, tools: any[], options?: any) => Promise<LLMResponse> {
  return async () => ({
    content: response,
    stopReason: 'stop',
...
describe('Agent (extra)', () => {
  ...
  describe('run with tool calls', () => {
    it('executes a tool and returns final text response', async () => {
      const agent = new Agent(undefined as any, [createTool('mytool')], createTestConfig());
      let callNum = 0;
      agent.setLLMProvider(async (prompt, tools) => {
        if (callNum === 0) {
          callNum++;
          return { content: '', stopReason: 'stop', usage: { input: 0, output: 0, total: 0, cost: { input: 0, output: 0, total: 0 } }, toolCalls: [{ name: 'mytool', arguments: { x: 1 }, id: 'c1' }] };
        } else {
          return { content: 'Result after tool', stopReason: 'stop', usage: { input: 2, output: 3, total: 5, cost: { input: 0, output: 0, total: 0 } }, toolCalls: [] };
        }
      });
  ...
  describe('queues', () => {
    it('steer() enqueues and hasQueuedMessages reflects it', () => {
      ...
      agent.steer({ role: 'user', content: [{ type: 'text', text: 'steer' }], timestamp: Date.now() });
    ...
    it('followUp() enqueues', () => {
      const agent = new Agent(undefined as any, [], createTestConfig());
      agent.followUp({ role: 'user', content: [{ type: 'text', text: 'follow' }], timestamp: Date.now() });
      expect(agent.hasQueuedMessages()).toBe(true);
  ...
  describe('config effects on strategy', () => {
    it('uses react strategy by default (parallel)', () => {
      const agent = new Agent(undefined as any, [], createTestConfig({ toolExecutionStrategy: 'parallel' }));
      expect((agent as any).strategy.constructor.name).toBe('ReActLoopStrategy');
    });

    it('uses simple strategy for sequential', () => {
      const agent = new Agent(undefined as any, [], createTestConfig({ toolExecutionStrategy: 'sequential' }));
      expect((agent as any).strategy.constructor.name).toBe('SimpleLoopStrategy');
    });

    it('respects explicit loopStrategy config', () => {
      const agent = new Agent(undefined as any, [], createTestConfig({ loopStrategy: 'plan-solve' }));
      expect((agent as any).strategy.constructor.name).toBe('PlanSolveLoopStrategy');
    });
  });
