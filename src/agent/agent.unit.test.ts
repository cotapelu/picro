...
describe('Agent (unit)', () => {
  describe('constructor', () => {
    ...
    it('should assign memoryStore when provided in config', () => {
      const memStore = { get: () => null, put: () => { }, delete: async () => { } };
      const agent = new Agent(undefined as any, [], defaultConfig({ memoryStore: memStore as any }));
  ...
  describe('queue management', () => {
    it('steer enqueues to steeringQueue', () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      const turn = { role: 'user' as const, content: [{ type: 'text' as const, text: 'Steer' }], timestamp: Date.now() };
      agent.steer(turn);
    ...
    it('followUp enqueues to followUpQueue', () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      const turn = { role: 'user' as const, content: [{ type: 'text' as const, text: 'FollowUp' }], timestamp: Date.now() };
      agent.followUp(turn);
    ...
    it('hasQueuedMessages reflects both queues', () => {
      ...
      agent.steer({ role: 'user', content: [{ type: 'text', text: 'A' }], timestamp: Date.now() });
      expect(agent.hasQueuedMessages()).toBe(true);
      agent.clearSteeringQueue();
      expect(agent.hasQueuedMessages()).toBe(false);
      agent.followUp({ role: 'user', content: [{ type: 'text', text: 'B' }], timestamp: Date.now() });
      expect(agent.hasQueuedMessages()).toBe(true);
    });
  ...
  describe('setModel', () => {
    ...
    it('sets providers when given a model', () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      const model: Model = { id: 'm', name: 'M', api: 'openai', provider: 'openai', baseUrl: '', reasoning: false, input: ['text'], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 1000, maxTokens: 100 };
  ...
  describe('run errors', () => {
    ...
    it('should reject if no LLM provider set', async () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      agent.setModel(undefined);
      await expect(agent.run('test')).rejects.toThrow('LLM provider not set. Provide a model or setModel() first.');
    });
  });
  ...
  describe('resume errors', () => {
    ...
    it('should reject if no LLM provider set', async () => {
      const agent = new Agent(undefined as any, [], defaultConfig());
      const model: Model = {
        id: 'm', name: 'M', api: 'openai', provider: 'openai', baseUrl: '', reasoning: false,
      ...
      agent.setModel(undefined);
      await expect(agent.resume()).rejects.toThrow('LLM provider not set. Provide a model or setModel() first.');
    });
    ...
    it('should reject if last turn is assistant and no queued messages', async () => {
      ...
      await expect(agent.resume()).rejects.toThrow('Cannot resume: last message is assistant and no queued messages');
    });
  ...
  describe('stream errors', () => {
    it('throws if no stream provider set', async () => {
      ...
      await expect(gen.next()).rejects.toThrow('LLM stream provider not set. Provide a model or setModel() first.');
    });
  });
  ...
  describe('public getters', () => {
    ...
    it('getMaxRounds returns config.maxRounds', () => {
      const agent = new Agent(undefined as any, [], defaultConfig({ maxRounds: 15 }));
