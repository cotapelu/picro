...
describe('AgentLoop (extra)', () => {
  ...
  describe('run', () => {
    ...
    it('stops after maxRounds reached', async () => {
      ...
      const result = await loop.run('Keep', new MessageQueue(), new MessageQueue());
      expect(result.success).toBe(false);
      expect(result.stopReason).toBe('max_rounds');
    });

    it('handles LLM throw', async () => {
      const llm = async (context: Context, options?: any) => { throw new Error('llm error'); };
      loop = new AgentLoop(
        createConfig(),
        emitter,
        toolExecutor,
      ...
      const result = await loop.run('Fail', new MessageQueue(), new MessageQueue());
      expect(result.success).toBe(false);
    ...
    it('aborts via abortable provider', async () => {
      const abortableLLM = (context: Context, options?: any) => new Promise<LLMResponse>((resolve, reject) => {
    ...
    it('emits error event on LLM failure', async () => {
      const llm = async (context: Context, options?: any) => { throw new Error('boom'); };
      loop = new AgentLoop(
