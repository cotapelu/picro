...
const dummyStreamProvider = async (context: Context, options?: any): Promise<AsyncIterable<any>> => {
  return {
...
function defaultConfig(overrides: Partial<AgentConfig> = {}): AgentConfig {
  return {
    ...
    contextBuilder: { maxTokens: 128000, reservedTokens: 4096, minMessages: 1, enableMemoryInjection: true },
...
describe('AgentLoop Hooks Branch Coverage', () => {
  ...
  describe('transformContext error', () => {
    it('catches transformContext error and returns failure', async () => {
      ...
      const configWithTransform = defaultConfig({ transformContext: badTransform });
      loop = new AgentLoop(
        configWithTransform,
        emitter,
        toolExecutor,
