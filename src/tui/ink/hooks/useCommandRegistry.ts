...
export function useCommandRegistry(ctx: CommandContext) {
  ...
  const handleCommand = useCallback(async (commandId: string, slashArgs?: string) => {
    const { runtime, addToast, setActiveModal } = ctx;
    ...
    switch (commandId) {
      ...
      case 'export':
        ...
        try {
          ...
          // Build HTML
          const escapedTitle = `Session Export`;
          let html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapedTitle}</title>`;
          html += `<style>body{font-family:sans-serif;max-width:800px;margin:auto;padding:20px}.user{background:#e3f2fd}.assistant{background:#e8f5e9}.tool{background:#fff3e0}</style>`;
          html += `</head><body><h1>${escapedTitle}</h1><p>CWD: ${cwd}</p><p>Messages: ${sessionMsgs.length}</p>`;
      ...
      case 'import':
        ...
        break;
      case 'share':
        // Share current session as GitHub gist
        try {
      ...
      case 'compact':
        try {
          ...
          if (typeof session.compact === 'function') {
            ...
            const customInstructions = parts.length > 0 ? args : undefined;
      ...
      case 'paste':
        ...
        try {
          ...
          setActiveModal(null); // This will update input via context; we'll handle differently
          // We need to access input setter; this will be handled by caller
          return 'paste';
        } catch (err: any) {
  ...
  }, [runtime, messages, addToast, setActiveModal, footerProvider]);
