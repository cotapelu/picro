...
export interface TreeSelectorTreeNode {
  name: string;
  path: string;
  isDirectory: boolean;
  children?: TreeSelectorTreeNode[];
}

export interface TreeSelectorOptions {
  root: TreeSelectorTreeNode;
  onSelect?: (node: TreeSelectorTreeNode) => void;
  onCancel?: () => void;
}

export class TreeSelector implements UIElement, InteractiveElement {
  private root: TreeSelectorTreeNode;
  private expandedDirs: Set<string> = new Set();
  private visibleNodes: { node: TreeSelectorTreeNode; depth: number }[] = [];
  private selectedIndex: number = 0;
  private onSelect?: (node: TreeSelectorTreeNode) => void;
  ...
  private flatten(node: TreeSelectorTreeNode, depth: number): void {
