...
- ✅ Các modals: CommandPalette, SettingsSelector, ModelSelector, SessionSelector, ThinkingModal, HelpModal, LoginModal, etc.
- ✅ Các components: Header, Footer, MessageList, MessageItem, InputBox
- ✅ Các molecule/organism: Armin, BashOutput, Tool execution cơ bản (chưa đầy đủ)
...
- ✅ Enhanced `Footer` component: dynamic display of cwd, session name, model, thinking level, token stats (input/output/cache/cost), auto-compact indicator, hints
...
### Đang thiếu / cần hoàn thiện tiếp theo
- ❌ AssistantMessage molecule: render text + thinking blocks (collapsible, italic), handle streaming updates
- ❌ ToolExecution molecule: custom renderers (renderCall/renderResult), image handling, expansion, background colors, setExpanded(), setShowImages(), setImageWidthCells()
- ❌ BashExecution molecule: render command output, exit code, truncation indicator
- ❌ MessageItem component: need to orchestrate rendering of User, Assistant, Tool, Bash, Custom, CompactionSummary, BranchSummary components
- ❌ Compaction/retry UI in chat (loader + countdown)
- ❌ Extension system: ExtensionUIContext implementation (select/confirm/input/notify, setWidget/setHeader/setFooter, custom overlay, autocomplete provider, theme getters/setters, tools expanded)
- ❌ Command handlers for remaining slash commands: `/scoped-models`, `/clone`, `/tree` (already modal but needs navigation logic), `/share`, `/copy`, `/name`, `/session`, `/changelog`, `/hotkeys`, `/quit`, `/reload`, `/compact`
- ❌ Command palette integration with slash commands/autocomplete
- ❌ Autocomplete system: fd-based path completion, slash command suggestions, prompt templates, extension commands, skills
- ❌ Resource loading display on startup: show loaded skills, prompts, extensions, themes with scope groups and diagnostics
- ❌ Version check & package update notification on startup
- ❌ Signal handlers (SIGTERM, SIGHUP, SIGCONT) & graceful shutdown
- ❌ External editor integration (`app.editor.external`)
- ❌ Clipboard image paste (`app.clipboard.pasteImage`)
- ❌ Tool output expansion toggle (connected to settings & UI)
- ❌ Thinking block visibility toggle (connected to settings & UI)
- ❌ Message queue display (steering/follow-up) with dequeue hint
- ❌ Footer dynamic update on model change (tested but may need fine-tuning)
- ❌ Custom header/footer/widgets from extensions
- ❌ Custom editor component support from extensions
- ❌ Output guards (sanitize/truncate)
- ❌ Performance metrics display in footer
