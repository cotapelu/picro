...
- ✅ `BashExecution` component placeholder (rendered as assistant text).
...
- ✅ MessageItem: ToolExecution and BashExecution integrated
- ✅ Compaction/retry UI in chat (status line)
- ⏳ Extension system: basic binding implemented (notify, setWidget, pasteToEditor, theme getters/setters, tools expanded). Dialogs (select/confirm/input), custom overlay, autocomplete provider, header/footer replacement still pending.
- ✅ Command handlers: `/scoped-models`, `/clone`, `/tree` (navigation logic implemented), `/share`, `/copy`, `/name`, `/session`, `/changelog`, `/hotkeys`, `/quit`, `/reload`, `/compact` (most implemented, minor stubs remain)
- ✅ Command palette integration (shows built-in, extension commands, skills, templates)
- ✅ Autocomplete system: fd-based path completion; slash command suggestions; prompt templates; extension commands; skills
- ✅ Resource loading display on startup: show counts of loaded skills, prompts, extensions, themes in header
- ✅ Version check on startup (notify if update available)
- ✅ Signal handlers (SIGTERM, SIGHUP) & graceful shutdown
- ✅ External editor integration (`app.editor.external`) – Ctrl+E opens $EDITOR
- ✅ Clipboard image paste (`/paste` command)
- ✅ Tool output expansion toggle (Ctrl+Shift+X)
- ✅ Thinking block visibility toggle (Ctrl+Shift+H)
- ✅ Message queue display (steering/follow-up) with dequeue hint (Ctrl+E to edit - partial)
- ✅ Footer dynamic update on model change (use currentModel from hook)
- ❌ Custom header/footer/widgets from extensions
- ❌ Custom editor component support from extensions
- ✅ Output guards (sanitize/truncate) – for tool and bash outputs
- ✅ Performance metrics display in footer – CPU/RSS averages
- ❌ Unit tests for core utilities and UI molecules
